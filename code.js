...
var MemTotal;
promPC.instantQuery("node_memory_MemTotal_bytes").then((res) => {
  if (res["result"][0]) {
...
function setGaugeValue(gauge, value, elementID, goodValue, warningValue) {
  ...
  element =
    document.getElementById(elementID).children["1"].children[1].children[0];
  if (value < goodValue) {
...
const updateGaugeValue = () => {
  promPC
    .instantQuery(
      '{__name__=~"aida_sys_scpuuti|aida_sys_sgpuuti|nvidia_smi_utilization_gpu_ratio|aida_sys_srtssfps|aida_sys_memuti|node_memory_MemAvailable_bytes|aida_temp|aida_temp_tgpu|nvidia_smi_temperature_gpu|aida_sys_suptime_total|node_boot_time_seconds"}',
    )
    .then((res) => {
      console.log("Then in prom PC instant query, result is: ", res);
      for (const metric of res.result) {
        switch (metric["metric"]["name"]) {
          ...
          case "node_boot_time_seconds":
            ...
            date2.setTime(
              new Date().getTime() - metric["value"]["value"] * 1000,
            );
          ...
          case "node_memory_MemAvailable_bytes":
            ...
            element =
              document.getElementById("ramUsageGauge").children["1"].children[1]
                .children[0];
            if (memUsage < 80) {
              element.style.fill = "var(--good)";
  ...
  promPC
    .instantQuery(`node_hwmon_temp_celsius{sensor='${tempSensor}'}`)
    .then((res) => {
      if (res["result"][0]) {
        if (res["result"][0]) {
          setGaugeValue(
...
const updateRpiValue = () => {
  promRPI
    .instantQuery("rpi_temperature_c")
    .then((res) => {
      document.getElementById("rpi-temp").textContent =
        Math.round(res.result[0]["value"]["value"] * 10) / 10;
      if (
        res.result[0]["value"]["value"] > 0 &&
        res.result[0]["value"]["value"] < 65
      ) {
        document.getElementById("rpi-temp").style.color = "var(--good)";
      } else if (
        res.result[0]["value"]["value"] >= 65 &&
        res.result[0]["value"]["value"] < 75
      ) {
    ...
    socket.addEventListener("message", async function (event) {
      try {
        ...
        const svgResponse = await fetch(
          weatherIconFolderLink + weatherIcon + ".svg",
        );
...
//FPS From linux desktop
async function getFPSWS() {
  let socket;

  async function connectWebSocket() {
    socket = new WebSocket(websocketFPSServer);

    socket.addEventListener("open", () => {
      console.log("WebSocket FPS connected");
    });

    socket.addEventListener("message", async function (event) {
      try {
        const FPS = Math.round(event.data);
        if (FPS >= 50 || FPS == 0) {
          document.getElementById("fps").style.color = "var(--good)";
