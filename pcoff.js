...
const updateRpiValue = () => {
  promRPI
    .instantQuery('{__name__=~"rpi_temperature_c|node_boot_time_seconds"}')
    .then((res) => {
      for (const metric of res.result) {
        switch (metric["metric"]["name"]) {
          case "rpi_temperature_c":
            document.getElementById("rpi-temp").textContent =
              Math.round(metric["value"]["value"] * 10) / 10 + "°";
            if (metric["value"]["value"] > 0 && metric["value"]["value"] < 65) {
              document.getElementById("rpi-temp").style.color = "var(--good)";
            } else if (
              metric["value"]["value"] >= 65 &&
              metric["value"]["value"] < 75
            ) {
              document.getElementById("rpi-temp").style.color =
                "var(--warning)";
            } else {
          ...
          case "node_boot_time_seconds":
            document.getElementById("uptime").textContent = formatTime(
